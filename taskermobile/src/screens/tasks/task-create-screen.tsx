...
import {TaskCreateScreenProps} from '@/types/react-navigation';
import {
  useCreateTask,
...
export default function TaskCreateScreen({navigation}: TaskCreateScreenProps) {
  ...
  const form = useForm({
    defaultValues: {
      title: '',
      description: '',
      dueDate: getISOString(DateTime.fromJSDate(new Date()).plus({days: 2})),
      reminderTime: getISOString(
        DateTime.fromJSDate(new Date()).plus({days: 1}),
      ),
      status: 'planned' as 'planned',
    },
    validators: {
      onSubmit: CreateTaskApiRequestSchema,
    },
          ...
          {field => (
            <>
              ...
              <Text style={styles.selectedDate}>
                Selected Date:{' '}
                {DateTime.fromISO(field.state.value).toLocaleString(
